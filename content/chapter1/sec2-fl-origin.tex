...
\end{equation*}
其中$\mathcal{X}_k$为数据样本的特征空间，$\mathcal{Y}_k$为数据样本的标签空间 (Sample Label Space) \index{样本的标签空间, Sample Label Space}，$\mathcal{I}_k$为数据样本编号空间。那么有\cite{Yang_2019_VFL,vfl}
\begin{itemize}
\item 横向联邦学习:
\begin{equation*}
