...
\end{array}
\end{equation}
其中符号$\expectation$表示关于分布的期望，$\theta$是模型参数，也就是我们的学习目标；$\mathcal{P}$是参与训练的节点的分布，这里我们假设了总共有$K$个节点；$w_k$是节点$k$的权重，一般与节点持有的数据量正相关；$\ell_k$是节点$k$上模型训练的损失函数(Loss Function)；$x, y$分别表示数据的特征与数据的标签。
\item 纵向联邦学习:
\begin{equation*}
