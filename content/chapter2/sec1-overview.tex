...
\end{array}
\end{equation}
我们很容易看出~\eqref{eq:fl-basic-constraint}~与~\eqref{eq:fl-basic}~这两种格式的等价性。格式~\eqref{eq:fl-basic-constraint}~在分布式优化中被称作共识问题(Consensus Problem)。这种格式的好处是，目标函数变成了分块可分离的(Block Separable)。目标函数的分块可分离性对于设计可并行化的算法是巨大的便利。我们会在本文随后的章节中进一步发现，约束优化的格式~\eqref{eq:fl-basic-constraint}~及其变体，更有利于我们对算法进行理论分析，进而对算法的改进做出指导。
