...
通过定理~\ref{thm:fedsplit-correctness}以及定理~\ref{thm:feddr-convergence}，我们可以看到分裂算法的确能够收敛到我们想要求解问题的极值点或者稳定点，这相较于联邦平均算法\texttt{FedAvg}~\ref{algo:fedavg}，联邦邻近算法\texttt{FedProx}~\ref{algo:fedprox}~是极大的进步。其代价，或者说缺点，就是出了模型参数本身，还有一些额外的辅助参数需要维护，增加了相关的存储以及运算的负担。
