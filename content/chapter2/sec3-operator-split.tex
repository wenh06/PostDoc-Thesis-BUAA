...
\begin{rem}
我们把定理~\ref{thm:fedsplit-correctness}应用到一个一般的最小二乘问题$f_k(\theta_k) = \lVert A_k \theta_k + b_k \rVert^2,$ $A_k$列满秩，$k \in [K],$ 那么我们有
\begin{itemize}
    \item $\theta^*_{\texttt{FedGD}} = \left( \sum\limits_{k=1}^K A_k^TA_k G \right)^{-1}\left( \sum\limits_{k=1}^K GA_k^Tb_k \right),$ 其中矩阵$G = \sum\limits_{i=0}^{R-1} ( I - \frac{1}{\mu} A_k^TA_k )^i$
    \item $x^*_{\texttt{FedProx}} = \left( \sum\limits_{k=1}^K \left( I - P_k \right) \right)^{-1} \left( \mu \sum\limits_{k=1}^K P_kA_k^Tb_k \right)，$ 其中矩阵$P_k = ( I + \frac{1}{\mu} A_k^TA_k )^{-1}$
\end{itemize}
上面的$\theta^*_{\texttt{FedGD}}, x^*_{\texttt{FedProx}}$分别是算法\texttt{FedGD}以及\texttt{FedProx}的理论收敛点。但是，实际上，这个最小二乘问题真正的最优解应该是
\begin{equation*}
...
我们需要重新考虑的优化问题~\eqref{eq:fl-basic}~的等价转换。我们把这个问题改写为如下的共识优化问题(注意与式~\eqref{eq:fl-basic-constraint}~的细微差别)
\begin{equation}
