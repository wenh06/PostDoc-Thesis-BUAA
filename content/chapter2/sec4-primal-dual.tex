...
\begin{equation}
\label{eq:al-dual}
\text{maximize} \quad G(\Lambda) := \inf\limits_{\Theta} \mathcal{L}(\Theta, \Lambda)
\end{equation}
可以得到解这种形式的问题算法的一般迭代格式
\begin{equation}
\label{eq:al-iter}
\begin{aligned}
\theta_k^{(t+1)} & = \argmin_{\theta_k} \mathcal{L}_k(\theta^{(t)}, \theta_k, \lambda_k^{(t)}) \\
\lambda_k^{(t+1)} & = \lambda_k^{(t)} + \frac{1}{s} (\theta_k^{(t+1)} - \theta^{(t)})
\end{aligned}
\end{equation}
