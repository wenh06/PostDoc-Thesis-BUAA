...
% \SetAlgoNoLine
% \DontPrintSemicolon
{\bfseries Initiation:} global parameters $\theta^{(0)} \in \operatorname{dom}(f)$, step size $s = \frac{1}{\mu} > 0$, $\alpha \in (0, 2)$, error bound for inexact proximal operator $\varepsilon_{k,0} \geqslant 0$ \\
\hspace{2em}Init server: $\overline{\theta}^{(0)} = \widetilde{\theta}^{(0)} = \omega^{(0)} = \theta^{(0)} \in \R^d$\\
\hspace{2em}Init clients: $\omega_k^{(0)} = \theta^{(0)}$, $\theta_k^{(0)} \approx \prox_{f_k, \mu}(\omega_k^{(0)})$, $\widehat{\theta}_k^{(0)} = 2\theta_k^{(0)} - \omega_k^{(0)}, ~ \forall k \in [K]$\;
\For{$t = 0, 1, \cdots, T-1$}{
...
\caption{联邦DR分裂算法FedDR\cite{tran2021feddr}伪代码}
\label{algo:feddr}
\end{algorithm}
