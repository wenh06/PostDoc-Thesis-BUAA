...
\For{each round $t = 0, 1, \cdots, T-1$}{
    ...
    \Indp
    $\Delta^{(t)} \gets \operatorname{aggregate} \left( \left\{ \Delta_{k}^{(t)} \right\}_{k \in \mathcal{S}^{(t)}} \right) \quad \left( \text{e.g.} \frac{1}{|\lvert \mathcal{S}^{(t)} \rvert} \sum\limits_{k \in \mathcal{S}^{(t)}} \Delta_{k}^{(t)} \right)$\;
